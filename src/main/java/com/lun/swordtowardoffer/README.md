...
在一个m * n的棋盘的每一格都含有一个礼物，每个礼物都有一定的价值（价值大于0）。你可以从棋盘的左上角开始拿格子里的礼物，并每次向左或向下移动一格，直到到达棋盘的右下角。给定一个棋盘及其上面的礼物，请计算你最多能拿到多少价值的礼物？

例如：下面棋盘，若沿着1-12-5-7-7-16-5，即可拿到最大价值为53的礼物。
...
[MaxValueGift](MaxValueGift.java)
...
tag:数学
	...
	把只包含因子2、3和5的数称作为丑数（Ugly Number)。求从小到大的顺序的第1500个丑数。例如，6、8都是丑数，但14不是，为它包含因子7。习惯上把1当作第一个丑数。
...
- 方法一：蛮力算法，一一测试匹配，时杂度O(mn)
- 方法二：使用栈，后进先出，时杂度O(m+n)，空杂度O(mn)，与方法一相比，空间换时间
- 方法三：计算两链长度差n，然后长链先走n步。走完n步，两链齐步走，直到碰到同一节点为止，时杂度O(m+n)，空杂度明显比方法二的少。

[FindTheSameNode](FindTheSameNode.java)
